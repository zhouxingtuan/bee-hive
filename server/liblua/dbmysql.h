...
#define DEFINE_CREATE_REMOVE_MYSQL()\
public:\
	DBMysqlPool m_DBMysqlPool;\
	DBMysql* createMysql(void){ return m_DBMysqlPool.create(); }\
	bool removeMysql(DBMysql* pDBMysql){ return this->removeMysql(pDBMysql->getHandle()); }\
	bool removeMysql(uint32 handle){\
		DBMysql* pDBMysql;\
		pDBMysql = m_DBMysqlPool.get(handle);\
		if(NULL != pDBMysql){\
			pDBMysql->closeMysql();\
			m_DBMysqlPool.idle(handle);\
			return true;\
		}\
		return false;\
	}\
