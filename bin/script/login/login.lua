...
function login:onInitialize(pNode, param)
	self.m_pNode = pNode
	-- 创建数据库
...
function login:login(request, response, rpcReq, connectHandle)
	...
	-- 把password解密
	password = self.m_pNode:decrypt(password, #password, "")
...
function login:generateSession(uid, name, password)
	...
	local magic = self.m_pNode:hash(temp, #temp)
	local session = {
		uid = uid;
		magic = magic;
		time = currentTime;
...
function login:getRegisterModuleIndex(name)
	local moduleIndex = self.m_pNode:hash(name, #name)
